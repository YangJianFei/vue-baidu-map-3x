...
<template>
  <div>
    <label v-for="label in labels" :key="label.type">
      <input name="controlType" type="radio" :checked="controlType === label.type" @change="controlType = label.type">{{
        label.name }}
    </label>
    <baidu-map v-if="controlType === 'scaleRule'" class="map" :center="{ lng: 116.404, lat: 39.915 }" :zoom="zoom"
    ...
    <baidu-map v-else-if="controlType === 'copyright'" class="map" :center="{ lng: 116.404, lat: 39.915 }" :zoom="zoom">
      <bm-copyright
        :copyrights="[{ id: 1, content: 'Copyright Message', bounds: { ne: { lng: 110, lat: 40 }, sw: { lng: 0, lat: 0 } } }, { id: 2, content: '<a>我是版权信息</a>' }]">
      </bm-copyright>
    </baidu-map>
