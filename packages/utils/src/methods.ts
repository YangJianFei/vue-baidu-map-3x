 ...
 * @LastEditTime: 2025-09-17 09:03:07
...
import { CopyrightControl, CopyrightInstance, MarkerInstance } from "../typing";
import { ControlsEnum } from "./constant";
import { getBounds, getIcon, getLabel, getPoint, getSize } from "./create";
  ...
  [
    ...
    [
      ...baseControlMethod,
      {
        ...
        method: "addCopyright",
        customMethod: (
          originInstance: Ref<CopyrightInstance | undefined>,
          newValue: CopyrightControl[]
        ) => {
          ...
          newValue?.forEach?.((copyright) => {
            ...
            originInstance?.value?.addCopyright?.(
              deleteEmptyKey({
                ...copyright,
                bounds,
              })
  ...
  [
    ...
    [
      ...baseControlMethod,
      {
        key: "icon",
        method: "setIcon",
        format: (icon) => getIcon(icon),
      },
      {
        key: "position",
        method: "setPosition",
        format: (position) => getPoint(position?.lng, position?.lat),
      },
      {
        key: "title",
        method: "setTitle",
      },
      {
        ...
        customMethod: (originInstance: Ref<MarkerInstance | undefined>, newValue) => {
          if (newValue) {
            originInstance.value?.enableDragging?.();
          } else {
            originInstance.value?.disableDragging?.();
          }
        },
      },
      {
        key: "enableMassClear",
        method: "enableMassClear",
        customMethod: (originInstance: Ref<MarkerInstance | undefined>, newValue) => {
          if (newValue) {
            originInstance.value?.enableMassClear?.();
          } else {
            originInstance.value?.disableMassClear?.();
          }
        },
      },
      {
